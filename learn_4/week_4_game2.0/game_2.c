...
void game_2()
{
	...
	//棋盘  -初始化
	init_board2(board, ROW, COL);
	//打印棋盘
	Display_board2(board, ROW, COL);
	...
	while (1)
	{
		...
		//玩家走
		player_do2(board, ROW, COL);
		Display_board2(board, ROW, COL);
		//判断输赢
		res = IsWin_Fail2(board, ROW, COL);
		if (res != 'c')
			break;
		//清空屏幕
		system("cls");
		//电脑走
		computer_do2(board, ROW, COL);
		Display_board2(board, ROW, COL);
		//判断输赢
		res = IsWin_Fail2(board, ROW, COL);
		if (res != 'c')

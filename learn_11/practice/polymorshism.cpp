...
void printVFTable(VFPtr vfTable[])
{
	...
	int i = 0;
	while (vfTable[i] != nullptr)
	{
		VFPtr f = vfTable[i++];	//vs2020改了,虚函数表末尾不再是nullptr,不能用这个方法
		cout << f << ": ";
		f();
	}
...
int main()
{
	...
	Derive d;
	//cout << "base1:" << endl;
	//printVFTable((VFPtr*)(*(VFPtr**)&b1));
	//cout << "base2:" << endl;
	//printVFTable((VFPtr*)(*(VFPtr**)&b2));
	cout << "derive:base1" << endl;
	printVFTable((VFPtr*)(*(VFPtr**)&d));
	cout << "derive:base2" << endl;
	Base2* pb2 = &d;	//切片
	printVFTable((VFPtr*)(*(VFPtr**)pb2));
